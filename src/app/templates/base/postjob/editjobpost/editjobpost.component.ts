...
export class EditjobpostComponent implements OnInit {
    ...
    this.apiService.getRequest(url).subscribe(
      row => {
        if (row['status'] == 200 && row['ok'] == true) {
          ...
          // add validation
          if (this.patchFormValue.status == 'Draft')
          ...
          if (this.patchFormValue.attachments != '') {
              ...
              this.uploadedFileId.forEach(element => {
                this.apiService.getRequest(constant.apiurl + constant.fileupload + element)
                  .subscribe(responseData => {
                    ...
                    let edifiletype = responseData['body']['file_ext'];
                    ...
                    const fileTypeVal = fileTypeDefine;
          ...
          if (this.patchFormValue.question_id != '') {
  ...
  onChangeDevice(apivalue: string, e: any) {
    if (e) {
    ...
    this.PopupSuggestQuestionRef.afterClosed().subscribe(result => {
      ...
      if (result != '' && result != 'close') {
        result.forEach(element => {
          ...
          // let ckquestion = this.questions.indexOf(element);
          // console.log(ckquestion);
          // let i:number = 0;
  ...
  newjobpostFormSubmit(formData) {
    ...
    // console.log(this.newjobpostForm)
    ...
    if (this.newjobpostForm.valid) {
      ...
      if (this.notvalid) {
        const URL = constant.apiurl + constant.updatejobpost + this.job_id + '/';
        ...
        this.apiService.putRequest(URL, params).subscribe(
          row => {
