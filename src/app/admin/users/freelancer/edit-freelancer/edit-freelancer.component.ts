...
export class EditFreelancerComponent implements OnInit {
    ...
    this.apiService.getRequest(constant.apiurl + constant.get_user_details + this.queryUserID + '/').subscribe(
      data => {
        ...
        if (this.initialData.status === 200) {
          ...
          this.requestParams['profile']['title'] = this.title = this.initialProfileData.profile.title;
  ...
  freelancerFormSubmit(formData) {
    ...
    if (this.freelancerForm.valid) {
      ...
      this.requestParams['profile']['title'] = this.title = formData.title;
  ...
  showSuccess() {
    ...
    setTimeout(() => {
        this.router.navigate(['/admin/user-freelancers']);
  ...
  geterrorMsg(field) {
    if (field === 'phone') {
      return this.freelancerForm.controls[field].hasError('required')
        || this.freelancerForm.controls[field].hasError('minlength')
        || this.freelancerForm.controls[field].hasError('maxlength') ? 'Please enter valid phone number' : '';
    } else if (field === 'zipcode') {
      return this.freelancerForm.controls[field].hasError('required')
        || this.freelancerForm.controls[field].hasError('minlength')
        || this.freelancerForm.controls[field].hasError('maxlength') ? 'Please enter valid zip code' : '';
    } else {
      return this.freelancerForm.controls[field].hasError('required') ? 'Field is required' : '';
    }
  }
