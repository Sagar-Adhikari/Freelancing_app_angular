...
export class SecurityQuestionComponent implements OnInit {
  ...
  displayedColumns = ['index', 'user_name', 'question', 'status', 'action'];
	SecqtnDatabase: SecqtnpyHttpDao | null;
	data: SecqtnlistData[] = [];
	dataSource = new MatTableDataSource<SecqtnlistData>();
	customFilter:string;
	resultsLength = 0;
	...
	ngOnInit() {
		this.SecqtnDatabase = new SecqtnpyHttpDao(this.http);
...
export interface SecqtnApi {
	results: SecqtnlistData[];
	count: number;
}

export interface SecqtnlistData {
	id: any;
	user_name:string;
	question: string;
	status: string;
...
export class SecqtnpyHttpDao {
	...
	getPyapi(sort: string, order: string, page: number, filter: string=''): Observable<SecqtnApi> {
		const href = constant.apiurl + constant.adminsecqtnurl;
		
		const requestUrl =
		`${href}?search=${filter}&sort=${sort}&order=${order}&page=${page + 1}`;
		return this.http.get<SecqtnApi>(requestUrl, this.setHeaders());
	}
}