...
@Component({
  ...
  styleUrls: ['./edit-security-question.component.css']
})
export class EditSecurityQuestionComponent implements OnInit {
  ...
  constructor(public sanitizer:DomSanitizer, formbuilder:FormBuilder, private api : ApiService, private router:Router, @Inject(DOCUMENT) private document: HTMLDocument,
    ...
    this.secqtnForm = formbuilder.group({
      ...
      'sec_qtn_status' : [null, Validators.compose([Validators.required, this.noWhitespaceValidator])]              
    });
  }
  ...
  getEditinfos(){
    ...
    var url = constant.apiurl + constant.adminsecqtnurl+id+'/';   
    this.api.getRequest(url).subscribe(result => {
	    ...
	    this.showError();
	 	...
	 	this.secqtnForm.patchValue({
        'sec_qtn_name': this.results.body.question,
        'sec_qtn_status':this.results.body.status
      });
	 });
  }
  ...
  secqtnFormSubmit(formData) {
    if(this.secqtnForm.valid) {
          const id = this.route.snapshot.paramMap.get('id');  
          var href = constant.apiurl+constant.adminsecqtnurl+id+'/';  
          var params = {
