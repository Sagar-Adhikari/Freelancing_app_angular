...
export class EditCategoryComponent implements OnInit {
  ...
  getEditinfos(){
    ...
    var url = constant.apiurl + constant.admineditcategorylist+'/'+id;   
    this.api.getRequest(url).subscribe(result => {
     ...
     var parse = JSON.parse(this.results.body.attachments);
     if (parse != null && parse != '' && typeof parse != 'undefined') {
       this.pre_img = true;
       this.preview_img = constant.imgurl+parse["0"].file;
     }
	 });
  }
  ...
  saveCategory(datas){
    ...
    const id = this.route.snapshot.paramMap.get('id');  
    var href = constant.apiurl+constant.admineditcategorylist+'/'+id+'/';  
    this.api.putRequest(href,datas).subscribe(result => {
    },error => {
      this.errormessage = error.error.non_field_errors["0"];
      this.showError();
    },() => {
