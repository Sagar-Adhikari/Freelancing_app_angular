...
export class TestCategoriesComponent implements OnInit {
	...
	JobsDatabase: catspyHttpDao | null;
	data: CatslistData[] = [];
	dataSource = new MatTableDataSource<CatslistData>();
	customFilter:string;
	resultsLength = 0;
	...
	ngOnInit() {
		this.JobsDatabase = new catspyHttpDao(this.http);
...
export interface CatsApi {
	results: CatslistData[];
	count: number;
}

export interface CatslistData {
	id: any;
	name: string;
	created: string;
}
...
export class catspyHttpDao {
	...
	getPyapi(sort: string, order: string, page: number, filter: string=''): Observable<CatsApi> {
		const href = constant.apiurl + constant.admintestcategorylist;
