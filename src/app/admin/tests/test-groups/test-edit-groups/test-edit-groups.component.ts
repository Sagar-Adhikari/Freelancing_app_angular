...
@Component({
	selector: 'app-test-edit-groups',
	templateUrl: './test-edit-groups.component.html',
	styleUrls: ['./test-edit-groups.component.css']
})
export class TestEditGroupsComponent implements OnInit {
    ...
    getEditinfos(){
    	...
    	var url = constant.apiurl + constant.admintestgrouplist+id;   
    	this.api.getRequest(url).subscribe(result => {
    		...
    		this.groupForm.patchValue({
    			'category_id': this.result_to_form.body.category_id,
    			'group_name': this.result_to_form.body.title,
    			'group_desc': this.result_to_form.body.description,
    			'group_status':this.result_to_form.body.status
    		});
    	});
    }
    ...
    groupFormSubmit(formData) {
    	if(this.groupForm.valid) {
    		const id = this.route.snapshot.paramMap.get('id');
    		var href = constant.apiurl+constant.admintestgrouplist+id;
    		var params = {
