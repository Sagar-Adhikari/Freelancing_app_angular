...
export class HomeSettingsComponent implements OnInit {
  ...
  displayedColumns = ['index', 'option_key', 'option_value', 'action'];
  OptionsDatabase: OptionspyHttpDao | null;
  data: OptionslistData[] = [];
  dataSource = new MatTableDataSource<OptionslistData>();
  customFilter:string;
  resultsLength = 0;
  ...
  ngOnInit() {
    this.OptionsDatabase = new OptionspyHttpDao(this.http);
  	...
  	if (search != '' && typeof search != 'undefined') {
  		merge(this.sort.sortChange, this.paginator.page)
			...
			switchMap(() => {
				...
				return this.OptionsDatabase!.getPyapi(
...
export class OptionspyHttpDao {
  ...
  getPyapi(sort: string, order: string, page: number, filter: string=''): Observable<OptionsApi> {
    const href = constant.apiurl + constant.adminoptionlist;
    
    const requestUrl =
        `${href}?search=${filter}&sort=${sort}&order=${order}&page=${page + 1}`;
    return this.http.get<OptionsApi>(requestUrl, this.setHeaders());
  }
}
